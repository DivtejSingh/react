...
import { getAllAdminsApi, getAllteacherApi, searchAdminApi } from "../../../utils/service/AdminService";
import { useDispatch, useSelector } from "react-redux";
import { setAdmin } from "../../../store/Slice/AdminSlice";
import Pagination from "../../../components/Pagination";
import Loading from "../../../components/Loading";
import { deleteUserDataByID, searchteacherApi, searchUserApi, userStateUpdate } from "../../../utils/service/DashboardService";
import toast from "react-hot-toast";
import UserManagementModal from "../../../components/Modal/user-management/UserManagementModal";
...
export default function Teachermanagement() {
  ...
  const getAllTeachers = async () => {
    try {
      setLoading(true);
      const response = await getAllteacherApi({ pg: currentPage, items_per_page: itemsPerPage });
      if (response?.isSuccess) {
  ...
  const handleSearch = async (e) => {
    ...
    //   return(
    //     item.username.toLowerCase().includes(e.target.value.toLowerCase().trim()) ||
    //     item.email.toLowerCase().includes(e.target.value.toLowerCase().trim())
    //   )
    // })
    // setfilteredData(filtereddata);
    const response = await searchteacherApi({ search: e.target.value });
      dispatch(setAdmin(response));
  ...
  const downloadPDF = async () => {
    try {
      ...
      const response = await  getAllteacherApi({ pg: currentPage, items_per_page:1000 });
       const {data} = response;
  ...
  return (
    <>
      ...
      <div className="flex justify-between sm:flex-col sm:gap-y-2 md:flex-col md:gap-y-2 lg:flex-col lg:gap-y-5">
        <h1 className="text-3xl font-bold sm:text-sm md:text-md lg:text-3xl">Teacher Management</h1>
      ...
      <div className="flex items-center ">
        <AddTeacherModal onUserCreated={getAllTeachers}  items={adminItem} addAdminModalOpen={addAdminModalOpen} setAddAdminModalOpen={handleModalClose} />
      </div>
