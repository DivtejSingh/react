...
import { useGetDataQuery, useRandomColorQuery } from '../store/service/HomeService';
        ...
        <>
            <section className=''>
                ...
                {!isLoading && !isError && getVideoCategorys?.data && Object.keys(getVideoCategorys?.data).filter(category => {
                    ...
                    const isActiveCategory = category === params?.cat_name;
                    return (
                        <div style={{ backgroundColor: getBackgroundColor(category, randomColor?.data) }} key={category}>
                            {
                                isActiveCategory && getVideoCategorys?.data[category]?.map((isParent, parentIndex) => {
                                        ...
                                        return (
                                            <div key={parentIndex}>
                                                <div className='category_center px-4'>
                                                    <h1 style={{ color: getColor(category, randomColor?.data) }} className='text-white mt-10 inline-block'>{params?.cat_name}</h1>
                                                </div>
                                                {isParent?.content?.map((Sub_Cat, subIndex) => {
                                                    ...
                                                    if (Sub_Cat.count > 0) {
                                                        return (
                                                            <>
                                                                {/* <div className='sm:hidden onmobilepadding px-4 pb-10' key={subIndex}>
                                                                    <div className='category_center'>
                                                                        <h3 style={{ color: getColor(category, randomColor?.data) }} className='text-white mt-3 inline-block'>{Sub_Cat.cat_name}</h3>
                                                                    </div>
                                        ...
                                        return (
                                            <>
                                                {/* <div className='sm:hidden onmobilepadding px-4 pb-10' key={parentIndex}>
                                                    <div className='category_center'>
                                                        <h1 className='text-white mt-10 inline-block'>{params?.cat_name}</h1>
                                                    </div>
...
export default WithHomeLayout(ParentCat);
